#if UNITY_EDITOR
//-----------------------------------------------------------------------// <copyright file="AssetUtilities.cs" company="Sirenix IVS"> // Copyright (c) Sirenix IVS. All rights reserved.// </copyright>//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
...
namespace Sirenix.Utilities.Editor
{
    ...
    /// </summary>
    public static class AssetUtilities2
    {
        /// <summary>
        ...
        public static IEnumerable<UnityEngine.Object> GetAllAssetsOfType(Type type)
        {
            ...
            {
                string[] goGuids = AssetDatabase.FindAssets("t:Prefab");

                for (int i = 0; i < goGuids.Length; i++)
                {
                    string goPath = AssetDatabase.GUIDToAssetPath(goGuids[i]);
                    var gos = AssetDatabase.LoadAllAssetRepresentationsAtPath(goPath) as GameObject[];

                    foreach (var go in gos)
                    {
                        var components = new List<Component>();
                        go.GetComponents(type, components); // Consider using GetComponentsInChildren if performance is not a problem.
                        components.ForEach(c => componentListBuffer.Add(c));
                    }

                    for (int j = 0; j < componentListBuffer.Count; j++)
                    {
                        yield return componentListBuffer[j];
                    }
                }
            }
            else
