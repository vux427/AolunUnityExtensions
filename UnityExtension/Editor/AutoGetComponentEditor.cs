...
namespace EternalAliceExtensions{
static public class AutoGetComponentEditor{
        ...
        static public Type GetElementType(this SerializedProperty property)
        {
            ...
            if (property.isArray)
            {
                ...
                type = property.GetArrayElementAtIndex(0).GetUnderlyingType().ToString();
            }
            else
                type = property.GetUnderlyingType().ToString();
            
            return type.GetTypeFromAssembly();
        }
